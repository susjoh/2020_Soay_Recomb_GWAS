...
vector2Dendrogram <- function(haplotype.vector, verbose = TRUE, count.cutoff = NULL){
  ...
  if(!is.null(count.cutoff)) haplotype.counts <- subset(haplotype.counts, Freq >= count.cutoff)
  ...
  library(ggdendro)
  print(ggdendrogram(hc, rotate = T) + theme(axis.text.y = element_text(family = "mono", size = 12)))
  str(hc)
